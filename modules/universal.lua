...
local function serverHop(pointer, filter)
	...
	if not pointer then
		notif('Pealzware', 'Searching for an available server.', 2)
	end
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('Pealzware', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('Pealzware', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('Pealzware', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end
...
run(function()
		...
		if self:get(v) ~= 0 then
			...
			if self.localprio == 0 then
				...
				vape.Uninject = function()
					notif('Pealzware', 'No escaping the private members :)', 10)
				end
				if joined then
					task.wait(10)
				end
	...
	function whitelist:process(msg, plr)
		...
		if self.localprio > 0 and not self.said[plr.Name] and msg == 'helloimusinginhaler' and plr ~= lplr then
			...
			warningNotification('Pealzware', plr.Name..' is using pealzware!', 60)
	...
	function whitelist:update(first)
		...
		local whitelistloaded, err = pcall(function()
			self.textdata = game:HttpGet(PealzwareFunctions.ResolveApiUrl(""), true)
		end)
